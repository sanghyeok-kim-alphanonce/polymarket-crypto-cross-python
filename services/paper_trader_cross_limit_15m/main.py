...
class CrossingLimitStrategy:
    ...
    name = "crossing_limit_15m"
    description = "15분봉 전체 crossing에 taker 진입 (UNIT/2*UNIT/UNIT, 10회 제한)"
...
# =============================================================================
class PaperTraderCrossLimit15MService(AsyncServiceBase):
    """Paper Trader: 15분봉 Crossing Strategy (횟수 제한)"""
    ...
    async def run(self):
        logger.info("=" * 60)
        logger.info("Paper Trader Cross Limit 15M: 15분봉 전체 Crossing 전략")
        logger.info("=" * 60)
...
def main():
    service = PaperTraderCrossLimit15MService()
