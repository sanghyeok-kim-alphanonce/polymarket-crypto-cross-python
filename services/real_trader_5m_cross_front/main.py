...
class CrossingStrategy5MFront:
    ...
    name = "crossing_5m_front"
    description = "5분봉 전체 crossing에 GTC 진입 (10회 제한, 290s+ → final)"
    ...
    def should_enter_on_crossing(
        ...
        # 10회차 판정: count == 10 OR elapsed >= LATE_ENTRY_SECONDS (290초)
...
class RealTrader5MCrossFrontService(AsyncServiceBase):
    ...
    async def run(self):
        logger.info("=" * 60)
        logger.info("Real Trader 5M Cross Front: 5분봉 전체 Crossing 전략")
        logger.info("=" * 60)
        logger.info(f"Strategy: {STRATEGY_NAME}")
        logger.info(f"  Entry: 0s ~ {CROSSING_CUTOFF_SECONDS}s (5분 전체)")
        ...
        logger.info(f"  GTC price: {GTC_FIXED_PRICE}")
        ...
        if self.is_live_enabled:
            ...
            self.notify(
                ...
                f"5M: 0s~{CROSSING_CUTOFF_SECONDS}s ({CROSSING_LATE_ENTRY_SECONDS}s+ → final)\n"
                f"Max {CROSSING_MAX_COUNT}x | GTC @{GTC_FIXED_PRICE}\n"
                f"Live: ENABLED"
            )
        else:
