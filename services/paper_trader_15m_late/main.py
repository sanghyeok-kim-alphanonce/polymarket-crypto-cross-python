...
# =============================================================================
# Crossing Strategy (15M Late - Max5 + Speed Filter)
# =============================================================================
@dataclass
class TradeSignal:
    side: str
...
class CrossingLateStrategy:
    """
    15분봉 후반 Crossing 전략 (Max5 + 속도필터)
    - 진입 구간: 660~880초 (첫 진입 x, 이후 2x)
    - 헤지 구간: 880~890초 (x로 헤지)
    - 금지 구간: 890초~ (진입 안 함)
    - Max5: 4번 진입 후 5번째는 x로 헤지
    - 속도 필터: 20초 내 3회 crossing 시 x로 헤지
    """

    name = "crossing_15m_late"
    description = "15분봉 후반(11분~) crossing (Max5 + 속도필터)"
    ...
    def should_enter_on_crossing(
        ...
        4. 880~890초 → x로 헤지
        5. 4번 진입 후 5번째 → x로 헤지
        6. 20초 내 3회 crossing → x로 헤지
        7. 정상 진입 → 첫 진입 x, 이후 2x
        """
        ...
        if is_hedge:
            ...
            reason = f"CROSS15M_LATE_{crossing_direction.upper()} #{state['count']} @{elapsed_seconds}s"
        else:
...
# =============================================================================
class PaperTrader15MLateService(AsyncServiceBase):
    """Paper Trader: 15분봉 후반(11분~) Crossing Strategy"""
    ...
    async def run(self):
        logger.info("=" * 60)
        logger.info("Paper Trader 15M Late: 15분봉 후반(11분~) Crossing 전략")
        logger.info("=" * 60)
        logger.info(f"Strategy: {STRATEGY_NAME}")
        logger.info(f"  Entry: {CROSSING_MIN_ELAPSED_SECONDS}s(11분) ~ {CROSSING_ENTRY_CUTOFF_SECONDS}s(14분40초)")
...
def main():
    service = PaperTrader15MLateService()
